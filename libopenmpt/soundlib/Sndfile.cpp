...
CSoundFile::~CSoundFile()
{
	...
	if (m_channelWaveformStates)
	{
		if (::g_active_waveform_states == m_channelWaveformStates)
		{
			::g_active_waveform_states = nullptr;
		}
		delete[] m_channelWaveformStates;
		m_channelWaveformStates = nullptr;
	}
}
...
extern "C" {
#include "waveform_processor.c"
}

OPENMPT_NAMESPACE_BEGIN

void CSoundFile::InitWaveformStates()
{
	m_channelWaveformStates = new ::ChannelState[256];
	for(int i = 0; i < 256; i++)
	{
		::waveform_channel_init(&m_channelWaveformStates[i]);
	}
	::g_active_waveform_states = m_channelWaveformStates;
}

float CSoundFile::GetChannelWaveform(CHANNELINDEX channel, float* output)
{
	if(!::g_active_waveform_states || channel >= 256)
	{
		if (output)
		{
			for (int i = 0; i < 512; i++) output[i] = 0.0f;
		}
		return 0.0f;
	}
	return ::waveform_channel_process(&::g_active_waveform_states[channel], output);
}
