...
extern "C" {
	float g_PlayChannelPCM[128][128] = {0};

	EMSCRIPTEN_KEEPALIVE float* openmpt_module_get_channel_pcm_ptr(void* mod, int channel) {
		if (channel < 0 || channel >= 128) return nullptr;
		return g_PlayChannelPCM[channel];
	}
}
...
void CSoundFile::CreateStereoMix(int count)
{
	...
	// 差分提取真实多通道 PCM 的临时缓冲
	std::vector<mixsample_t> tempBuffer;
	if (count > 0) {
		tempBuffer.resize(count * 2);
		// 清空当前帧的全局通道 PCM 缓存
		std::memset(g_PlayChannelPCM, 0, sizeof(g_PlayChannelPCM));
	}

	for(uint32 nChn = 0; nChn < m_nMixChannels; nChn++)
	{
		CHANNELINDEX channel = m_PlayState.ChnMix[nChn];
		
		// 备份混音前的主缓冲区数据
		if (channel < 128 && count > 0) {
			std::memcpy(tempBuffer.data(), MixSoundBuffer, count * 2 * sizeof(mixsample_t));
		}

		if(MixChannel(count, m_PlayState.Chn[channel], channel, numChannelsMixed < m_MixerSettings.m_nMaxMixChannels)) {
			numChannelsMixed++;

			// 差分提取：当前缓冲区 减去 备份缓冲区，并转为单声道 float
			if (channel < 128 && count > 0) {
				int limit = count < 128 ? count : 128;
				for (int i = 0; i < limit; i++) {
					mixsample_t diffL = MixSoundBuffer[i * 2] - tempBuffer[i * 2];
					mixsample_t diffR = MixSoundBuffer[i * 2 + 1] - tempBuffer[i * 2 + 1];
					g_PlayChannelPCM[channel][i] = (float)(diffL + diffR) * 0.5f;
				}
			}
		}
	}
		...
		do
		{
			...
			if((nSmpCount = mixLoopState.GetSampleCount(chn, nrampsamples)) <= 0)
			{
				...
				*pOfsL += chn.nLOfs;
				...
				// Feed silence so the waveform/LED decays instead of freezing
				if (::g_active_waveform_states && channel < 256) {
					float silence_buf[256];
					for (int j = 0; j < 256; j++) silence_buf[j] = 0.0f;
					int rem = nsamples;
					while (rem > 0) {
						int chunk_size = rem > 256 ? 256 : rem;
						::waveform_channel_feed_float(&::g_active_waveform_states[channel], silence_buf, chunk_size);
						rem -= chunk_size;
					}
				}
			   ...
			   || (!chn.nRampLength && !(chn.leftVol | chn.rightVol)))  // Channel is completely silent
			{
				...
				if (::g_active_waveform_states && channel < 256) {
					float silence_buf[256];
					int to_feed = nSmpCount > 256 ? 256 : nSmpCount;
					for (int j = 0; j < to_feed; j++) silence_buf[j] = 0.0f;
					int rem = nSmpCount;
					while (rem > 0) {
						int chunk_size = rem > 256 ? 256 : rem;
						::waveform_channel_feed_float(&::g_active_waveform_states[channel], silence_buf, chunk_size);
			...
			else
			{
				...
				// 保存混音前的数据
				float temp_buf[512];
				int to_copy = nSmpCount > 256 ? 256 : nSmpCount;
				for (int j = 0; j < to_copy * 2; j++) {
					temp_buf[j] = (float)pbuffer[j];
				}
				...
				if (::g_active_waveform_states && channel < 256) {
					...
					for (int j = 0; j < to_copy; j++) {
						float left_diff = (float)pbuffer[2 * j] - temp_buf[2 * j];
						float right_diff = (float)pbuffer[2 * j + 1] - temp_buf[2 * j + 1];
						diff_buf[j] = (left_diff + right_diff) * 0.5f * (1.0f / 32768.0f);
					}
					::waveform_channel_feed_float(&::g_active_waveform_states[channel], diff_buf, to_copy);
					
					if (nSmpCount > 256) {
						float extra_silence[256];
						for (int j = 0; j < 256; j++) extra_silence[j] = 0.0f;
						int rem = nSmpCount - 256;
						while (rem > 0) {
							int chunk_size = rem > 256 ? 256 : rem;
							::waveform_channel_feed_float(&::g_active_waveform_states[channel], extra_silence, chunk_size);
							rem -= chunk_size;
						}
					}
				}
