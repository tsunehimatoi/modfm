...
// init
const initPromise = (typeof libopenmptPromise === 'function')
	? libopenmptPromise()
	: (libopenmptPromise && libopenmptPromise.ready ? libopenmptPromise.ready : Promise.resolve(libopenmptPromise));

initPromise
.then(res => {
	libopenmpt = res || libopenmptPromise
...
class MPT extends AudioWorkletProcessor {
	...
	process(inputList, outputList, parameters) {
		...
		if (actualFramesPerChunk == 0) {
			// 修复逻辑：在播放的前 0.5 秒内，忽略 read_float_stereo 返回 0 的情况，防止初始化未准备好导致连续跳歌
			const currentPos = libopenmpt._openmpt_module_get_position_seconds(this.modulePtr) || 0
			if (currentPos < 0.5) {
				return true
			}
			...
			if (error) {
				this.paused = true
				this.port.postMessage({cmd:'err',val:'Process',gen:this.gen||0})
			} else {
				this.port.postMessage({cmd:'end',gen:this.gen||0})
				this.paused = true
			}
			return true
		}
		...
		// ── SidWiz waveform processor (integrated inside libopenmpt) ─────
		if (this.sampleCountSinceEmit === undefined) {
			this.sampleCountSinceEmit = 0;
		}

		this.sampleCountSinceEmit += actualFramesPerChunk;
		if (this.sampleCountSinceEmit >= 1024) {
			this.sampleCountSinceEmit = 0;
			...
			// Allocate output buffer in libopenmpt memory if not done yet
			if (!this.wfOutPtr) {
				this.wfOutPtr = libopenmpt._malloc(WAVEFORM_SAMPLES * 4); // float is 4 bytes
				this.wfOutBuf = new Float32Array(libopenmpt.HEAPF32.buffer, this.wfOutPtr, WAVEFORM_SAMPLES);
			}

			// Read waveforms for all channels directly from openmpt
			for (let i = 0; i < this.channels; i++) {
				const intensity = libopenmpt._openmpt_module_get_channel_waveform(this.modulePtr, i, this.wfOutPtr);
				
				// 实时利用当前的 HEAP 构造视图以避免 Memory Growth Detached 错误
				const waveView = new Float32Array(libopenmpt.HEAPF32.buffer, this.wfOutPtr, WAVEFORM_SAMPLES);
				const wave = waveView.slice(); // 克隆
